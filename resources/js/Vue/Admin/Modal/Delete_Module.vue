    ...
    <Transition name="modal-fade">
        ...
        >
            ...
            <Transition name="modal-scale" appear>
                ...
                <form
                    @submit.prevent="deleteModuleForm"
                ...
                >
                    ...
                    <div class="w-full flex flex-col gap-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
                        ...
                        <div class="w-full flex items-center gap-1.5">
                            ...
                            <span v-if="props.totalQuestions === 0" class="text-xs text-gray-700">
                                No questions have been added yet.
                            </span>

                            <span v-else class="text-xs text-gray-700">
                                {{ props.totalQuestions }}
                                {{ props.totalQuestions === 1 ? 'Question' : 'Questions' }}
                            </span>
                    ...
                    <div class="w-full flex gap-3 mt-1">
                        ...
                        <button
                            type="button"
                            @click="closeModal"
                            class="deleteQuestionCancel w-1/3 py-2.5 rounded-md text-sm font-medium text-gray-600 border border-gray-300 hover:bg-gray-100 transition-colors cursor-pointer"
                        >
                            Cancel
                        </button>
...
const emit = defineEmits(['close', 'module_deleted'])


async function deleteModuleForm() {

    if (isSubmitting.value) return

    if (!props.moduleId) {
        message.value = 'Module ID is missing.'
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            emit('close')
            emit('module_deleted', data.deleted_module_id)
        } else {
            showToast.value = true
            message.value = data.message || 'Failed to delete module.'
        }

    } catch (err) {
        console.error(err)
        showToast.value = true
        message.value = 'Something went wrong while deleting the module.'
    } finally {
