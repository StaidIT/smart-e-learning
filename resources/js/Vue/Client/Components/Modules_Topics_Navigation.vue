            ...
            >
                ...
                <li v-if="module.order === expandedModuleOrder" class="cursor-pointer">
                    ...
                    <div
                        ...
                        class="px-5 py-[7px] flex items-center justify-between bg-[#4C1D95]/10 text-[#4C1D95] font-semibold border-l-[3px] border-[#4C1D95]"
                    >
                    ...
                    <ul class="flex flex-col bg-[#FBFAFE] border-l-[3px] border-[#4C1D95]">
                        ...
                        <template
                            v-for="topic in topics.filter(topic => topic.module_id === module.id)"
                            :key="topic.id"
                        >
                ...
                <li v-else class="cursor-pointer">
                    ...
                    >
                        ...
                        <span>
                            <span
                                :class="module.order === activeModuleOrder
                                    ? 'text-purple-400'
                                    : 'text-gray-400'"
...
const firstModule = firstTopic
    ? props.modules.find(module => module.id === firstTopic.module_id)
    : props.modules[0] ?? null
...
function getModuleFromTopic(topic) {
    ...
    return props.modules.find(
        module => module.id === topic.module_id
    ) ?? null
    ...
    observer = new IntersectionObserver(
        ...
        (entries) => {
            ...
            if (visible.length > 0) {
                ...
                const match = props.topics.find(
                    topic => topic.slug === visible[0].target.id
                )
